...
(
    ...
    theory_classroom_type     VARCHAR(32),
    experiment_classroom_type VARCHAR(32),
    practice_classroom_type   VARCHAR(32),
...
CREATE INDEX idx_course_category ON sc_course (category);
...
COMMENT ON COLUMN sc_course.category IS '课程类别（枚举：GENERAL_EDUCATION/MAJOR_COURSE/PRACTICE_COURSE）';
COMMENT ON COLUMN sc_course.property IS '课程属性（枚举：REQUIRED/ELECTIVE/LIMITED_ELECTIVE）';
COMMENT ON COLUMN sc_course.type IS '课程类型（枚举：THEORY/EXPERIMENT/PRACTICE/COMPUTER）';
COMMENT ON COLUMN sc_course.nature IS '课程性质（枚举：PUBLIC_COURSE/MAJOR_BASIC/MAJOR_CORE）';
...
COMMENT ON COLUMN sc_course.theory_classroom_type IS '理论课教室类型（枚举：NORMAL/MULTIMEDIA/LECTURE_HALL/LABORATORY/COMPUTER_ROOM/SMART/MUSIC/ART/SPORTS）';
COMMENT ON COLUMN sc_course.experiment_classroom_type IS '实验课教室类型（枚举）';
COMMENT ON COLUMN sc_course.practice_classroom_type IS '实践课教室类型（枚举）';
